...
- запуск автофиксеров при сохранении файла;
...
| `fixers`              | `FixerConfig[]`        | нет          | Команды автофиксеров. По умолчанию запускаются через `LintRunner: Run Fixers`; `run: "onSave"` также запускает их при сохранении. |
...
| `fixCommand`          | `FixerConfig`          | нет          | Legacy per-linter auto-fixer. Для новых конфигов лучше `fixers` на target.                        |
...
| Поле  | Тип                       | Обязательное | Описание                                                                 |
| ----- | ------------------------- | ------------ | ------------------------------------------------------------------------ |
| `run` | `"manual" \| "onSave"`   | нет          | По умолчанию `manual`. `onSave` запускает fixer при сохранении и вручную. |

Остальные поля такие же, как у `CommandConfig`.

## Command Variables

LintRunner подставляет переменные в `command`, `args`, `preCommands[*].command`, `preCommands[*].args`,
`fixers[*].command`, `fixers[*].args`, `fixCommand.command` и `fixCommand.args`.
...
`fixCommand` и `fixers` выполняются через `LintRunner: Run Fixers`. Если у fixer command задано `run: "onSave"`,
он также выполняется при сохранении matching файла. Для всех matching configs команды выполняются
последовательно. После фиксеров расширение запускает линтеры, чтобы обновить Problems.
